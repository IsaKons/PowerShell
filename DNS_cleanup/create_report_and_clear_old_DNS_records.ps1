...
#Cleanup DNS records requirements: 
#- records whose names contain UL or UD and 5 and 6 characters after. For example - BGSAUL00013 or BWRTUD00001
#- records with the same IP. Only the record  with the last registration date (no older than 1 month) is excluded.



$dry_run = $false # if you want to test script without removing records set up $true. Just create a report.
...
$path = ($MyInvocation.MyCommand.Path | Split-Path -Parent)  + "\Reports"    #Start script folder
...
Foreach ($item in $result_items2)
 ...
 ### Delete PTR records ###
$count = 0 # added pause for 5 min every 1000 records
                        ...
                        try
                            {
                                if (!$dry_run)
                                 {
                                    Remove-DnsServerResourceRecord -ZoneName $ReverseZoneName -ComputerName $DNSServer -InputObject $NodePTRRecord -Force  -ErrorAction Stop
                                  }
                                $Status = "Delete"
                                $count++
                            }
